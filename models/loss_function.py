# Codes are borrowed from https://github.com/mahmoodlab/PORPOISE
...
def nll_loss(h, y, c, alpha=0.0, eps=1e-7, reduction='mean'):
    if h.dim() == 3 and h.size(-1) == 1:
        h = h.squeeze(-1)
    assert h.dim() == 2, f"h must be [B, T], got {h.shape}"

    y = y.long().view(-1, 1)
    if c.dtype in (torch.float16, torch.float32, torch.float64):
        c = (c > 0.5).long().view(-1, 1)
    else:
        c = c.long().view(-1, 1)

    B, T = h.shape
    hazards = torch.sigmoid(h)                 # [B, T]
    S = torch.cumprod(1 - hazards, dim=1)      # [B, T]

    ones = torch.ones((B, 1), device=h.device, dtype=hazards.dtype)
    S_padded = torch.cat([ones, S], dim=1)     # [B, T+1]

    if torch.any((y < 0) | (y >= T)):
        raise ValueError(f"y out of range [0,{T-1}]: {y.min().item()}~{y.max().item()}")

    s_prev = torch.gather(S_padded, dim=1, index=y).clamp(min=eps)           # S(t-1)
    h_this = torch.gather(hazards,  dim=1, index=y).clamp(min=eps)           # h(t)
    s_this = torch.gather(S_padded, dim=1, index=y + 1).clamp(min=eps)       # S(t)

    uncensored_loss = (1 - c) * (-(torch.log(s_prev) + torch.log(h_this)))
    censored_loss   = c * (-(torch.log(s_this)))
    neg_l = censored_loss + uncensored_loss

    loss = (1 - alpha) * neg_l + alpha * uncensored_loss if alpha is not None else neg_l
